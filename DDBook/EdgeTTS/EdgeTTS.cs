...
namespace DDBook.EdgeTTS
{
    public class EdgeTTS : IDisposable
    {
        ...
        public async Task<Result> SynthesisAsync(string str, Action<string> log = null, string voice = "zh-CN-XiaoxiaoNeural",
            string pitch = "+0Hz", string rate = "+0%", string volume = "+100%")
        {
            var message = "X-Timestamp:" + Utils.GetFormatedDate() +
                          "\r\nContent-Type:application/json; charset=utf-8\r\nPath:speech.config\r\n\r\n";
            message += "{\"context\":{\"synthesis\":{\"audio\":{\"metadataoptions\":{\"sentenceBoundaryEnabled\":" +
            ...
            while (retryTimes < 3)
            {
                try
                {
                    ...
                    log?.Invoke($"[EdegTTS]接收用时:{stopwatch.Elapsed.TotalMilliseconds}ms");
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex.InnerException != null && ex.InnerException is SocketException &&
                        ...
                        log?.Invoke("[EdgeTTS]Azure服务器主动断开连接");
                    else
                        log?.Invoke("[EdgeTTS]Azure服务器连接异常");

                    log?.Invoke(ex.ToString());
                    log?.Invoke($"[EdegTTS]连接失败，开始第{retryTimes}次重试 :(");
                }
                //finally
                //{
        ...
        public Voice[] GetVoiceList()
        {
            ...
            var stringReader = new StreamReader(resStream);
            var resString = stringReader.ReadToEnd();

            return JsonConvert.DeserializeObject<Voice[]>(resString)!.OrderBy(a => a.Gender).ToArray();
        }
